...
- **Chat-history attached to ideas.** Half the plumbing exists: `messages` already records every text message, `events` already supports `idea_edited`, and the board has a `long`-description editor. What's missing: (a) a link between a `messages` row and an `idea_id` (add a nullable `idea_id` column to `messages`, or a join table), (b) an agent method like `attachMessageToIdea(messageId, ideaId)` and a summarizer pass that folds attached messages into `idea.long`, (c) router tool entries — `attach_to_idea` ("link this discussion to #3") and `enrich_idea` ("update #3 with what we just said"), (d) a board UI surface on the card to show the attached thread. Closely related to the title/brief polish entry above; both live in `agent.ts → addIdea` / a new `enrichIdea` path. User-flagged on 2026-05-01.
